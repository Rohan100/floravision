...
function HorizontalCard({ image, side, price }: props) {
    return (
        <div className='bg-white/5 backdrop-blur-sm border border-white/40 rounded-[4rem] grid md:grid-cols-2  px-8 md:py-4 py-6 items-center' >
            ...
            {
                side == 'left' ? <LeftCompoent image={image} /> : <RightComponent price={price}/>
                
            }
            {
                side == 'right' ?  <LeftCompoent image={image} /> : <RightComponent price={price}/>
            }
        </div>
...
function LeftCompoent({ image }: { image: string }) {
    return (
...
function RightComponent({ price }: { price: string }) {
    return (
        <div className='self-center w-4/5 m-auto'>

            <h3 className="md:text-4xl text-3xl font-bold text-white mb-2">
                For Your Desks Decorations
            </h3>
            <p className="text-white/70 mb-4 text-sm">
                These trendy plants fit even in minimal and perfectly to my desks workspace and it possible enhance!
            </p>
            <p className="md:text-4xl text-3xl font-bold text-white mb-4">Rs. {price}/-</p>
            <div className="flex gap-3">
                <button className="px-6 md:text-2xl text-xl w-fit py-3 bg-white/10 hover:bg-white/20 border border-white/30 rounded-lg text-white font-medium transition-all">
                    Explore
                </button>
